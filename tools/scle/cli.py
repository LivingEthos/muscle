"""
CLI - Command-line interface for SCLE.
...
def cli() -> None:
    """SCLE - Self-Correcting Loop Engine
...
@click.option(
    "--webhook-url", default=None, help="Webhook URL for notifications (or set SCLE_WEBHOOK_URL)"
)
...
def run(
    ...
    console.print(
        f"[bold]SCLE Session[/bold] - Task: {_truncate(task, MAX_TASK_PREVIEW_LENGTH)}..."
    )
    ...
    webhook_notifier = WebhookNotifier(webhook_url or os.environ.get("SCLE_WEBHOOK_URL"))
    ...
    try:
        ...
        with Progress(
            ...
            progress.add_task("Running SCLE...", total=None)
...
def history() -> None:
    ...
    table = Table(title="SCLE Sessions")
