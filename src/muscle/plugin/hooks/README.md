...
- `src/muscle/plugin/hooks/hooks.json` — read by **Claude Code** (subdir
  convention; lifecycle events: `SessionStart`, `UserPromptSubmit`, `Stop`).
- `src/muscle/plugin/hooks.json` — read by **Codex** (top-level
...
(`src/muscle/doctor.py:CLAUDE_HOOKS_PATH` / `CODEX_HOOKS_PATH`).
