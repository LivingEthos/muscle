...
class TestClaudePublisher:
    ...
    def test_publish_writes_to_agents_md_when_present(self) -> None:
        """Test that publish writes to both CLAUDE.md and AGENTS.md when both exist."""
        from tools.muscle.claude_publisher import ClaudePublisher
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            publisher = ClaudePublisher(tmpdir)
            result = publisher.publish(
                critical_rules=[{"text": "Shared rule", "score": 0.8, "validated_count": 3}],
            )

            assert result is True
            claude_content = claude_md.read_text()
            agents_content = agents_md.read_text()
            assert "Shared rule" in claude_content
            assert "Shared rule" in agents_content

    def test_publish_skips_agents_md_when_absent(self) -> None:
        """Test that publish succeeds with only CLAUDE.md present."""
        from tools.muscle.claude_publisher import ClaudePublisher
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            publisher = ClaudePublisher(tmpdir)
            result = publisher.publish(
                critical_rules=[{"text": "Solo rule", "score": 0.8, "validated_count": 3}],
            )

            assert result is True
            assert not (project / "AGENTS.md").exists()

    def test_pinned_sections_never_consolidated(self) -> None:
        """Test that pinned sections bypass M2.7 consolidation."""
        from unittest.mock import MagicMock

        from tools.muscle.claude_publisher import ClaudePublisher, SECTION_METHODOLOGY
    ...
    def test_publish_includes_pinned_block(self) -> None:
        """Test that publish always includes pinned Methodology/Delegation/Effort sections."""
        from tools.muscle.claude_publisher import ClaudePublisher
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "### Effort & Tool Guidance" in content

    def test_publish_pinned_appears_exactly_once(self) -> None:
        """Test idempotency: publishing twice produces pinned section exactly once."""
        from tools.muscle.claude_publisher import ClaudePublisher
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert content.count("### Delegation Protocol") == 1
